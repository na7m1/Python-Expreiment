...
```mermaid
graph TD
A[Start] -->|draft, crew| B((Ship))
B -->|is_worth_it()| C{decision}
C -->|True| D[Yes]
C -->|False| E[No]
...
- Python的类中的__init__方法是一个特殊方法，用于在创建类的实例时进行初始化操作。它允许在实例化对象时传递参数，并对对象进行初始化。
2. Python语言中如何继承父类和改写（override）父类的方法。

- 在Python中，继承父类并改写（override）父类的方法可以通过创建子类来实现。子类可以继承父类的方法，并且可以通过定义相同名称的方法来覆盖父类的方法。
3. Python类有那些特殊的方法？它们的作用是什么？请举三个例子并编写简单的代码说明。

- Python中类的特殊方法包括：

__init__: 在实例化对象时进行初始化操作

__str__: 定义对象的字符串表示形式，用于在使用print函数时打印对象

__len__: 返回对象的长度，常用于自定义类的长度计算

## 实验总结

通过这次实验，我巩固了对Python面向对象编程的理解，并学习了如何在Markdown中使用Mermaid语法来绘制程序流程图。