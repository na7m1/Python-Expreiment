...
def shorten_number(suffixes, base):
    def filter(item):
        try:
            ...
            return str(item)
        p = len(suffixes)
        while num >= base and p > 1:
            num = num // base
            p -= 1
        return f"{num}{suffixes[-p]}"
    return filter
```
...
- 函数式编程是一种编程范式，它将计算视为数学函数的计算，避免使用变量和可变状态。函数式编程强调函数的纯度、不可变性和无副作用。它通常使用高阶函数和不可变数据结构来构建程序。
2. 什么是lambda函数？请举例说明。

- Lambda函数是一种匿名函数，它可以在一行内定义并使用，通常用于需要一个函数，但只使用一次的情况。Lambda函数的语法为 lambda arguments: expression。以下是一个简单的Lambda函数示例：
...
- 高阶函数是指能接受其他函数作为参数，或者能够返回一个函数的函数。常用的高阶函数包括 map、filter、reduce 等。

- map 函数：对可迭代对象中的每个元素应用一个函数。
- filter 函数：用于过滤序列，返回符合条件的元素。
- reduce 函数：对一个序列中的元素进行累积操作。
```python
# 使用 map 函数
numbers = [1, 2, 3, 4, 5]
...
在本次实验中，我们学习了 Python 函数的基本用法、lambda 函数和高阶函数的使用，以及函数式编程的概念和实践。